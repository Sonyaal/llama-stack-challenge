...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const prompt = `
        ...
        The main compliance regulations you are looking for are not following California's energy efficiency requirements, incorrect soil conditions, missing information, MAJORLY incorrect grammar/spelling, and anything that violates California building regulation. 
    ...
    const response = await axios.post(
      ...
      {
        model: 'mistral', // or another model you've run via `ollama run <model>`
        stream: false,
        messages: [
          {
            role: 'system',
